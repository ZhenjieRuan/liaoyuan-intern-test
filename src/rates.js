...
const OVER_1500: number = 45;
const OVER_4500: number = 345;
const OVER_9000: number = 1245;
const OVER_35000: number = 7745;
const OVER_55000: number = 13745;
const OVER_80000: number = 22495;

module.exports.Rates = class Rates {
  rates: JSONObject;

  constructor(data: Array<JSONObject>) {
    this.rates = {};
    if (data.length === 1) {
      /* tax rates */
      this.rates = data[0];
  ...
  get_pension_rate(type: string): number {
  ...
  get_medical_rate(type: string): number {
    if (typeof this.rates[type] === 'object') {
  ...
  get_unemployment_rate(type: string): number {
    if (typeof this.rates[type] === 'object') {
  ...
  get_maternity_rate(type: string): number {
    if (typeof this.rates[type] === 'object') {
  ...
  get_injury_rate(type: string): number {
    if (typeof this.rates[type] === 'object') {
  ...
  between(num: number, lower: number, upper: number): boolean {
    return num > lower && num <= upper;
  }

  get_tax_amount(salary: number): number {
  ...
  calculate_insurances_details(salary: number, housing_fund_rate: number, type: string) {
  ...
  get_insurances_details(salary: number, average_salary: number, housing_fund_rate: number): JSONObject {
    var insurances_total: JSONObject = {};
