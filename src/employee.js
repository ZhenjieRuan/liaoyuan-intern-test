...
module.exports.Employee = class Employee {
  ...
  constructor(data: JSONObject) {
    if (data !== {}) {
      if (
          typeof data['姓名'] === 'string' &&
          typeof data['基本工资'] === 'number' &&
          typeof data['绩效评分'] === 'string' &&
          typeof data['公积金'] === 'number'
         )
      {
        this.name = data['姓名'];
  ...
  calculate_income_details(insurances_rates: Rates, tax: Rates, bonus: Rates, average_salary: number, csv: boolean) {
    // console.log(insurances_rates);
    var _bonus: number = bonus.get_bonus(this.get_rating());
    var insurances_details: JSONObject = insurances_rates.get_insurances_details(this.get_base_salary(), average_salary, this.get_housing_fund_rate());
    var insurances_total_individual: number = 0;
    var insurances_total_company: number = 0;
    ...
    var before_tax: number = this.get_base_salary() + _bonus - insurances_total_individual;
    var _tax: number = tax.get_tax_amount(this.get_base_salary() + _bonus - insurances_total_individual);
