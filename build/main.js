...
parse_csv(DS_PATH + '个税税率.csv', (err, result) => {
  if (err) {
...
parse_csv(DS_PATH + '五险费率.csv', (err, result) => {
  if (err) {
    return console.log(err);
  }
...
parse_csv(DS_PATH + '绩效工资标准.csv', (err, result) => {
  if (err) {
    return console.log(err);
  }
...
parse_csv(DS_PATH + '本市职工月平均工资.csv', (err, result) => {
  if (err) {
    return console.log(err);
  }
  ...
  while (!finished) {
    ...
    if (mode === 'manual') {
      ...
      parse_csv(DS_PATH + '员工名单.csv', (err, result) => {
        if (err) {
          return console.log(err);
        }
